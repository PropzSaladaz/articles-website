...
Before looking at cryptography, we first need to understand what an elliptic curve really is mathematically.

At this stage, I want you to keep everything you know, or saw in the previous article about private keys, public keys, encryption, and signatures aside. In the following sections we will focus only on understanding the mathematical object that ECC is built on.

Then we will go back to the cryptography again.


## 2. What Is an Elliptic Curve?

An elliptic curve is a special kind of mathematical curve. In other words, it is a function that can be drawn in a 2D plane.
...
Don't stress out if you can't see how the curve looks like from its equation. We will graph it in the next subsection.
...
## 4. Why the Curve Is Symmetric

Elliptic curves in this form are symmetric across the x-axis.
...
This symmetry will become important in the next article, when we define the **inverse** of a point. For now, the useful thing to remember is simple:
...
The answer is yes. That is where **elliptic curve point addition** begins.
