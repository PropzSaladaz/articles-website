...
# 1. Point Addition and the Elliptic Curve Group
...
The point at infinity, written `O`, is the special point that acts like zero in elliptic curve addition.
...
So `O` is the identity element of elliptic curve addition.
...
## 7. The Elliptic Curve Group

For an elliptic curve, the group is made from all valid points on the curve, plus the point at infinity `O`.
...
* normal curve points
* the special point at infinity `O`
...
## 8. Closure, Identity, Inverses, Associativity

Groups have some rules that we haven't talked about yet directly. You have probably already built an inuition for them, but its usually nice to lay them out explicitly so that you know they exist:
...
| Identity      | There is a point `O` such that `P + O = P`                          |
| Inverse       | Every point `P` has an inverse `-P` such that `P + (-P) = O`        |
...
Adding `O` does not change the point.
...
## 9. Why the Group Structure Matters
...
* there is a zero-like point `O`
* every point has an inverse
* repeated addition is unambiguous
...
Next, we can define scalar multiplication more carefully.
