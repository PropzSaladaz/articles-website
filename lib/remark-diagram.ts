...
const diagramTypes = new Set(['flow', 'branch']);
  ...
  return (tree: any) => {
    visit(tree, (node: any) => node.type === 'containerDirective', (node: any) => {
      if (node.name === 'diagram') {
        ...
        const label = typeof node.label === 'string' ? node.label.trim() : 'Diagram';
      ...
      if (node.name === 'flow') {
        const label = typeof node.label === 'string' ? node.label.trim() : 'Flow diagram';
      ...
      if (node.name === 'outcomes') {
        const label = typeof node.label === 'string' ? node.label.trim() : 'Outcome diagram';
        const items = listItems(node);
        const source = node.children?.find((child: any) => child.type === 'paragraph');
